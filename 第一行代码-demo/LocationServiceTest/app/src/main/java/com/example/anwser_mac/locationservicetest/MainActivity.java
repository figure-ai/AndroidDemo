package com.example.anwser_mac.locationservicetest;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (ContextCompat.checkSelfPermission(MainActivity.this, READ_PHONE_STATE)!=PackageManager.PERMISSION_GRANTED) {
                ...
                if (grantResults.length > 0) {
                    for (int result : grantResults) {
                        if (result != PackageManager.PERMISSION_GRANTED) {
                            Toast.makeText(this, "必须同意所有所用授权才能使用本程序", Toast.LENGTH_SHORT).show();
                        }
                    }
                    requestLocation();
                } else {
